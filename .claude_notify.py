...
def play_sound():
    # WSL2: use powershell.exe (available on PATH in WSL2)
    subprocess.run(
        ['powershell.exe', '-c', '[System.Media.SystemSounds]::Exclamation.Play(); Start-Sleep -Milliseconds 500'],
        capture_output=True
    )
...
def show_popup(msg):
    ...
    subprocess.run(['powershell.exe', '-c', ps], capture_output=True)
