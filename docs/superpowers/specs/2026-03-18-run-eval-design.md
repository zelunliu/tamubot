...
Default is `False` for backwards compatibility. Only the benchmark passes `return_timing=True`.
...
`timing_ms` key names match the decorated function names (e.g. `router_node`, `retrieval_node`). Recurrent-only fields are `None` for non-recurrent rows.
...
Inserted after existing timing columns (`pipeline_ms`, `generator_ms`, `total_ms`):
...
Add entries for: `is_recurrent`, `router_ms`, `retrieval_ms`, `anchor_ms`, `eval_search_ms`, `schedule_filter_ms`, `merge_ms`.
