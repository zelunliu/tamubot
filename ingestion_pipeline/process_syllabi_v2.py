"""
ingestion_pipeline/process_syllabi_v2.py

V2 syllabus pipeline — granular steps, flat versioned output, combined logs.

All files for every version live flat in the step root directory, with _vNNN
in the filename (e.g. 202611_CSCE_670_600_46627_v001.md). Each step maintains
a single combined log (CSV + JSONL) that accumulates across all runs.

Steps:
  0. Copy source PDF  → v2_step0_source/{stem}_v001.pdf
  1. PDF → Markdown   → v2_step1_markdown/{stem}_v001.md   (fitz annotated)
  2. Strip boilerplate→ v2_step2_boilerplate/{stem}_v001.md + _v001_stripped.md
  3. Chunk + LLM meta → v2_step3_chunked/{stem}_v001.json

Each step reads the LATEST versioned output of the previous step as its input
(highest _vNNN), unless a specific input file is supplied.

Usage:
    # All steps, new run version:
    python ingestion_pipeline/process_syllabi_v2.py --pilot --new-run

    # Individual steps (resume current version):
    python ingestion_pipeline/process_syllabi_v2.py --pilot --step 0 --new-run
    python ingestion_pipeline/process_syllabi_v2.py --pilot --step 1
    python ingestion_pipeline/process_syllabi_v2.py --pilot --step 2
    python ingestion_pipeline/process_syllabi_v2.py --pilot --step 3

    # Force re-process (ignores skip-existing):
    python ingestion_pipeline/process_syllabi_v2.py --pilot --step 2 --force

    # Single PDF:
    python ingestion_pipeline/process_syllabi_v2.py --pdf tamu_data/raw/simple_syllabus_20260305/202611_CSCE_670_600_46627.pdf --new-run
"""
...
from ingestion_pipeline.chunker_v2 import chunk_markdown  # noqa: E402
...
# Source PDFs only from simple_syllabus_* directories (not syllabi/)
...
def _simple_syllabus_dirs() -> list[Path]:
    """Return all simple_syllabus_* dirs sorted newest-first."""
    return sorted(RAW_ROOT.glob("simple_syllabus_*"), reverse=True)


def find_pilot_pdfs() -> list[Path]:
    """Find PDFs for the 5 pilot courses from simple_syllabus_* dirs only."""
...
def find_department_pdfs(department: str) -> list[Path]:
    """Find all PDFs for a department from simple_syllabus_* dirs only."""
    seen: dict[str, Path] = {}
...
def step0_copy_source(
    ...
    """Copy source PDF into step0 root with _vNNN suffix.

    Skips if ANY version of this stem already exists (not just the current one),
    since the source PDF doesn't change between runs.
    """
...
def step1_to_markdown(
    ...
    """Convert PDF to fitz-annotated Markdown, output as {stem}_vNNN.md."""
    stem = pdf_path.stem
    # Strip any existing _vNNN suffix from stem (if reading from step0 output)
...
def step2_strip_boilerplate(
    ...
    """Strip boilerplate from Markdown. Writes {stem}_vNNN.md + _vNNN_stripped.md."""
    versioned_name = f"{stem}_{version}.md"
...
# ── Step 3: Chunk + LLM Metadata ─────────────────────────────────────────────

def _extract_metadata(client, annotated_text: str) -> tuple[dict, str]:
    """LLM call: extract course metadata from font-annotated text.

    Output is always small (~400 tokens) regardless of syllabus length.
    Returns (metadata_dict, error_str).
    """
...
def step3_chunk_and_save(
    ...
    """LLM parses font-annotated text → clean markdown + metadata; chunk and save."""
    versioned_name = f"{stem}_{version}.json"
    out_file = STEP3_ROOT / versioned_name

    if skip_existing and out_file.exists():
        return {"file": versioned_name, "status": "skipped"}

    try:
        # Heuristic: convert font-annotated text → clean markdown (no LLM, no size limits)
        clean_markdown = annotated_to_clean_markdown(filtered_text)
        # LLM: extract metadata only (small output, always reliable)
        meta, llm_error = _extract_metadata(client, filtered_text)
        chunks = chunk_markdown(clean_markdown)
        ...
        header_chunks = sum(1 for c in chunks if c.get("header_level", 0) > 0)
        input_link = (
            StepLogger.hyperlink(input_file, input_file.name)
            if input_file and input_file.exists()
            else (str(input_file) if input_file else "")
        ...
        row = {
            ...
            "clean_markdown_tokens": len(clean_markdown) // 4,
...
def process_pdf(
    ...
    """Run the requested pipeline steps for one PDF."""
    # Canonical stem (no _vNNN suffix, no extension)
    stem = re.sub(r"_v\d{3}$", "", pdf_path.stem)
    print(f"  {pdf_path.name}")
    result: dict = {"file": pdf_path.name, "status": "ok"}

    md_text: str = ""        # step1 output: font-annotated text
    filtered_text: str = "" # step2 output: boilerplate-stripped annotated text
    ...
    # ── Step 1 ──
    if 1 in run_steps:
        # For step1, always use the original source PDF
        ...
        # Load latest step1 output as input for downstream steps
    ...
    if 3 in run_steps:
        ...
        r = step3_chunk_and_save(client, stem, filtered_text, input_file, version, loggers[3], skip_existing)
        if r["status"] == "error":
            result.update({"status": "error", "error": r.get("error", "")})
...
def main():
    parser = argparse.ArgumentParser(
        description="V2 syllabus pipeline — versioned filenames, combined step logs"
    )
    ...
    parser.add_argument("--new-run", action="store_true",
                        help="Increment version counter (vNNN+1). Only meaningful with --step 0 or all steps.")
    ...
    # --new-run creates a new version; only triggers when step 0 runs
    force_new = args.new_run and (0 in run_steps or args.step is None)
    version = resolve_version(force_new=force_new)
    print(f"Pipeline version: {version}  (steps: {run_steps})")

    # Ensure all step roots exist and write run meta
    step_roots = {0: STEP0_ROOT, 1: STEP1_ROOT, 2: STEP2_ROOT, 3: STEP3_ROOT}
    cli_args = vars(args)
    for s in run_steps:
        step_roots[s].mkdir(parents=True, exist_ok=True)
        write_run_meta(step_roots[s], version, cli_args)

    # Build descriptive named loggers (combined across all versions)
    log_names = {
        0: "step0_source_copy_log",
        1: "step1_pdf_to_markdown_log",
        2: "step2_boilerplate_strip_log",
        3: "step3_chunk_metadata_log",
    }
    loggers = {
    ...
    for pdf in pdfs:
        r = process_pdf(client, pdf, version, loggers, skip_existing, run_steps)
        results.append(r)
